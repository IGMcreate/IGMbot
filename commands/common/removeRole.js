...
export default {
    ...
    async execute({ inter }) {
        if (inter.member.permissionsIn(inter.channel).has(PermissionsBitField.Flags.ManageRoles) ||
            inter.member.permissionsIn(inter.channel).has(PermissionsBitField.Flags.Administrator)) {
            const role = inter.options.getRole('role');
            const member = inter.options.getMember('target');
            member.roles.remove(role);
            inter.editReply(`Success!`);
