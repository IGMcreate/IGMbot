...
export default async function interactionHandler(inter) {
    // Only defer once if not already deferred or replied
    const deferOnce = async () => {
    ...
    const replyError = async (message) => {
        await deferOnce();
        return inter.editReply({
            embeds: [new EmbedBuilder().setColor('#ff0000').setDescription(message)],
            ephemeral: true
        });
    };
    ...
    if (inter.type === InteractionType.ApplicationCommand) {
        ...
        const DJ = global.client.config.opt.DJ;
        const command = global.client.commands.get(inter.commandName);

        if (!command) {
            await replyError('Error!');
            global.client.slash.delete(inter.commandName);
            return;
        }

        if (command.permissions && !inter.member.permissions.has(command.permissions)) {
            return replyError('You do not have the proper permissions to execute this command');
        }
        ...
        if (DJ.enabled && DJ.commands.includes(command) &&
            ...
            return replyError(`This command is reserved for members with \`${DJ.roleName}\``);
        }

        if (command.voiceChannel) {
            if (!inter.member.voice.channel) return replyError('You are not in a voice channel');

            const meVC = inter.guild.members.me.voice.channel;
            if (meVC && inter.member.voice.channel.id !== meVC.id) {
                return replyError('You are not in the same voice channel');
            }
        }

        await command.execute({ inter, client: global.client });
    }
    ...
    if (inter.type === InteractionType.MessageComponent) {
        ...
        if (ignoredButtonIds.includes(fileOfButton)) return;

        const queue = useQueue(inter.guild);

        if (customId.command === 'Mayor') {
            const page = customId.pagenum || 1;
            const { default: mayorCommand } = await import('../../commands/skyblock/Mayor.js');
            await mayorCommand.execute({ inter, client: global.client, page });
        }

        else if (customId.command === 'Accessories') {
            const page = customId.pagenum || 1;
            const playerREP = customId.player || '';
            const profileREP = customId.profile || '';
            const { default: AccessoriesCommand } = await import('../../commands/skyblock/Accessories.js');
            await AccessoriesCommand.execute({ inter, client: global.client, page, playerREP, profileREP });
        }

        else if (fileOfButton && !customId.command) {
            await deferOnce();
            const buttonModule = await import(`../../src/buttons/${fileOfButton}.js`);
            const button = buttonModule.default;
            if (button) await button({ inter, client: global.client, customId, queue });
        }
    }
    ...
    if (inter.type === InteractionType.ApplicationCommandAutocomplete) {
        const command = global.client.commands.get(inter.commandName);
        if (command && command.autocomplete) {
            await command.autocomplete(inter, global.client);
        }
    }
}
