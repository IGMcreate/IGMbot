...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (number) {
            ...
            const trackname = tracksArray[index]?.title;
            //const trackname = queue.tracks.toArray()[index].title

            if (!trackname) return await inter.editReply({ content: `This track dose not seem to exist ${inter.member}...  try again ?`, ephemeral: true });
